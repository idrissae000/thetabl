  ...
  return (
    <div className="group relative overflow-hidden border border-border-gold aspect-[4/3] flex flex-col justify-between p-5 cursor-default transition-all duration-300 hover:border-gold/50">
      ...
      <div className="relative z-10">
        <div className="font-serif text-3xl text-gold leading-none tracking-wide">{code}</div>
        <div className="font-sans text-xs font-medium tracking-ultra uppercase text-text-muted mt-1">{name}</div>
      </div>

      <div className="relative z-10">
        <div className="font-sans text-xs tracking-ultra uppercase text-text-muted/80">{label}</div>
      </div>
...
export function ComingSoonCityCard({ code }: CityCardProps) {
  return (
    ...
    >
      ...
      <div className="font-sans text-[10px] font-semibold tracking-ultra uppercase text-text-faint/80 border border-text-faint/25 px-3 py-1">
        Coming Soon
      </div>
    </div>
  )
