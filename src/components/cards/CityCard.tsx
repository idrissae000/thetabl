...
// Dark gradient used when no city image is available
const fallbackBg =
...
export function ActiveCityCard({ code, name, label, image }: CityCardProps) {
  return (
    <div className="group relative overflow-hidden border border-border-gold aspect-[4/3] flex flex-col justify-between p-5 cursor-default transition-all duration-300 hover:border-gold/50">
      ...
      {/* Bottom label */}
      <div className="relative z-10">
        <div className="font-sans text-[9px] tracking-ultra uppercase text-text-muted/70">
          {label}
        </div>
      </div>
...
export function ComingSoonCityCard({ code, name }: CityCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="font-sans text-[8px] tracking-ultra uppercase text-text-faint/60 border border-text-faint/20 px-3 py-1">
        Coming Soon
      </div>
    </div>
  )
